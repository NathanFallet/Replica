...
import fr.zabricraft.replica.utils.Metrics.Graph;
import fr.zabricraft.replica.utils.Metrics.Plotter;
...
public class Replica extends JavaPlugin {
	...
	public void onEnable(){
			...
			if(i <= ga){
				...
				if(data != null){
					for(String sk : data.getKeys(false)){
						g.getSigns().add(new Location(Bukkit.getWorld(data.getString(sk+".world")), data.getInt(sk+".x"), data.getInt(sk+".y"), data.getInt(sk+".z")));
					}
				}
				games.add(g);
			}
		...
		Bukkit.getScheduler().scheduleSyncRepeatingTask(this, new Runnable(){
			...
			public void run() {
				for(Game g : games){
					if(g.getState().equals(GameState.IN_GAME)){
						...
						if(g.getState().equals(GameState.START_COUNT)){
							...
							if(g.getCurrentCountValue() == 0){
								g.start();
							}else if(g.getCurrentCountValue() == 60 || g.getCurrentCountValue() == 30 || g.getCurrentCountValue() == 20 || g.getCurrentCountValue() == 10 || g.getCurrentCountValue() <= 5){
								PacketContainer pc = new PacketContainer(PacketType.Play.Server.TITLE);
								pc.getTitleActions().write(0, TitleAction.TITLE);
								pc.getChatComponents().write(0, WrappedChatComponent.fromText("§a"+g.getState().getText().replaceAll("%d", g.getCurrentCountValue()+"")));
								for(UUID uuid : g.getPlayers()){
									Player p = Bukkit.getPlayer(uuid);
									p.sendMessage("§e"+messages.get("chat-start-count").replaceAll("%d", g.getCurrentCountValue()+""));
									try {
					...
					lines.add("§e");
					lines.add("§e§lPlugin by ZabriCraft");
		...
		try {
	        ...
	        Graph graph1 = metrics.createGraph("Number of pictures");
	        graph1.addPlotter(new Plotter(pictures.size()+" picture"+(pictures.size() > 1 ? "s" : "")){
				@Override
				public int getValue() {
					return 1;
				}
	        ...
	        Graph graph2 = metrics.createGraph("Number of games");
	        graph2.addPlotter(new Plotter(games.size()+" game"+(games.size() > 1 ? "s" : "")){
				@Override
				public int getValue() {
					return 1;
				}
	        ...
	        metrics2.addCustomChart(new Metrics2.SimplePie("pictures_count") {
				@Override
	        ...
	        metrics2.addCustomChart(new Metrics2.SimplePie("games_count") {
				@Override
				public String getValue() {
